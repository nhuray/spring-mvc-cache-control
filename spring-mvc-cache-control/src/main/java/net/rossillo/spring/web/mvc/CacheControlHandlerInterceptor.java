...
public class CacheControlHandlerInterceptor extends HandlerInterceptorAdapter implements HandlerInterceptor {
    ...
    private boolean usePragmaNoCacheHeader = true;
    ...
    protected final void assignCacheControlHeader(
        ...
        if (cacheControlHeader != null) {
            ...
            List<CachePolicy> cachePolicies = Arrays.asList(cacheControl.policy());
            if (cachePolicies.contains(NO_CACHE) && usePragmaNoCacheHeader){
                response.setHeader(PRAGMA_HEADER, NO_CACHE.policy());
            }
        }
    }
    ...
    /**
     * True to set a Pragma header equals to 'no-cache' when a {@link CacheControl} annotation is present
     * on a handler and {@link CachePolicy} NO_CACHE is set; false otherwise.   Defaults to true.
     *
     * @param usePragmaNoCacheHeader <code>true</code> to set an Pragma header equals to 'no-cache' when a
     * <code>CacheControl</code> annotation is present on a handler; <code>false</code> otherwise
     */
    public void setUsePragmaNoCacheHeader(boolean usePragmaNoCacheHeader) {
        this.usePragmaNoCacheHeader = usePragmaNoCacheHeader;
    }
}
